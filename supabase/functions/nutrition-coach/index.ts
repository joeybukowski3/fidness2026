 ...
 properties:{
  ...
  proposedItems:{
   ...
   items:{
    type:'object',
    properties:{
     name:{type:'string'},quantity:{type:'string'},meal:{type:'string',enum:MEALS},
    ...
    required:['name','quantity','meal','calories','protein','carbs','fat','notes','confidence']
   }
  }
 },
 required:['reply']
...
USER:${message}
Return only valid JSON with this shape: {"reply":"string","summary":"string optional","proposedItems":[{"name":"string","quantity":"string","meal":"Breakfast|Lunch|Dinner|Snack","calories":0,"protein":0,"carbs":0,"fat":0,"notes":"string","confidence":"high|medium|low"}]}.`}
...
async function callGemini(apiKey:string,model:string,input:string,structured:boolean){
 const body:any={model,input};
 if(structured)body.response_format={type:'text',mime_type:'application/json',schema:SIMPLE_SCHEMA};
 const r=await fetch('https://generativelanguage.googleapis.com/v1beta/interactions',{method:'POST',headers:{'Content-Type':'application/json','x-goog-api-key':apiKey},body:JSON.stringify(body)});
 const data=await r.json().catch(()=>({}));
 return{ok:r.ok,status:r.status,data,text:textOf(data)};
}
Deno.serve(async req=>{
 ...
 try{
  ...
  const model=Deno.env.get('NUTRITION_GEMINI_MODEL')||Deno.env.get('GEMINI_MODEL')||'gemini-3.1-flash-lite',input=prompt(message,conversation,context);
  let result=await callGemini(apiKey,model,input,true);
  const reason=clean(result.data?.error?.message,1000);
  if(!result.ok&&result.status===400&&/invalid argument|schema|response.?format/i.test(reason)){
   console.warn('Structured nutrition response rejected; retrying plain JSON.',reason);
   result=await callGemini(apiKey,model,input,false);
  }
  if(!result.ok){console.error(result.data);return response({error:result.data?.error?.message||`Gemini request failed (${result.status}).`},result.status===429?429:502,origin)}
  if(!result.text)return response({error:'Gemini returned an empty response.'},502,origin);
  return response(normalized(parseJson(result.text)),200,origin);
